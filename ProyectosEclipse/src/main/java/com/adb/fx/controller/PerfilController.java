...
public class PerfilController {
    ...
    public void initialize() {
        ...
        // Mostrar sección de información por defecto
        mostrarInformacion();
        
        // Inicializar estadísticas vacías (se llenarán con microservicios)
    ...
    private void cargarDatosUsuario() {
        ...
        lblRolBadge.setText("🎯 " + usuarioActual.getRol());
    ...
    /**
     * Verifica si el usuario es capitán para mostrar/ocultar opciones
     */
    private void verificarRolCapitan() {
        if (usuarioActual == null) return;
        
        String rol = usuarioActual.getRol() != null ? usuarioActual.getRol().toLowerCase() : "jugador";
        boolean esCapitan = rol.contains("capitan") || rol.contains("capitán");
        
        // Mostrar u ocultar card de crear equipo según el rol
    ...
    /**
     * Cambia la tab activa y muestra la sección correspondiente
     */
    ...
    private void inscribirseATorneo() {
        ...
        confirmacion.setHeaderText("¿Deseas inscribirte a este torneo?");
        confirmacion.setContentText("Torneo: " + torneoSeleccionado);

        confirmacion.showAndWait().ifPresent(response -> {
            if (response == ButtonType.OK) {
                try {
                    // TODO: Implementar endpoint de inscripción en el backend
                    mostrarAlerta("✅ ¡Te has inscrito exitosamente al torneo!\n\n" +
    ...
    private void crearEquipo() {
        ...
        String rol = usuarioActual.getRol() != null ? usuarioActual.getRol().toLowerCase() : "jugador";
        if (!rol.contains("capitan") && !rol.contains("capitán")) {
            mostrarAlerta("❌ Necesitas ser capitán para crear equipos.\n\n" +
                         "Ve a la sección 'Solicitudes' para solicitar el rol de capitán.", 
                         Alert.AlertType.ERROR);
            return;
        }

        try {
            // TODO: Implementar creación de equipo en el backend
    ...
    private void solicitarCapitan() {
        ...
        if (razon.isEmpty() || razon.length() < 20) {
            mostrarAlerta("Por favor explica detalladamente por qué deberías ser capitán (mínimo 20 caracteres)", 
                         Alert.AlertType.WARNING);
            return;
        }
        ...
        confirmacion.setHeaderText("¿Enviar solicitud para ser Capitán?");
        confirmacion.setContentText("Tu solicitud será revisada por un administrador.");

        confirmacion.showAndWait().ifPresent(response -> {
            if (response == ButtonType.OK) {
                try {
                    // TODO: Implementar endpoint de solicitud en el backend
                    System.out.println("=== SOLICITUD DE CAPITÁN ===");
                    System.out.println("Usuario: " + usuarioActual.getIdUsuario());
                    System.out.println("Razón: " + razon);
                    
                    mostrarAlerta("✅ Solicitud enviada exitosamente\n\n" +
                                "Tu solicitud para ser capitán ha sido enviada al administrador.\n" +
    ...
    private void solicitarArbitro() {
        ...
        if (experiencia.isEmpty() || experiencia.length() < 20) {
            mostrarAlerta("Por favor describe tu experiencia como árbitro (mínimo 20 caracteres)", 
                         Alert.AlertType.WARNING);
            return;
        }
        ...
        Alert confirmacion = new Alert(Alert.AlertType.CONFIRMATION);
        confirmacion.setTitle("Confirmar Solicitud");
        confirmacion.setHeaderText("¿Enviar solicitud para ser Árbitro?");
        confirmacion.setContentText("Tu solicitud será revisada por un administrador.");
        ...
        confirmacion.showAndWait().ifPresent(response -> {
            if (response == ButtonType.OK) {
                try {
                    // TODO: Implementar endpoint de solicitud en el backend
                    System.out.println("=== SOLICITUD DE ÁRBITRO ===");
                    System.out.println("Usuario: " + usuarioActual.getIdUsuario());
                    System.out.println("Experiencia: " + experiencia);
                    
                    mostrarAlerta("✅ Solicitud enviada exitosamente\n\n" +
                                "Tu solicitud para ser árbitro ha sido enviada al administrador.\n" +
                                "Recibirás una notificación cuando sea revisada.", 
                                Alert.AlertType.INFORMATION);
    ...
    private void eliminarCuenta() {
        ...
        confirmacion1.setTitle("⚠️ Eliminar Cuenta");
        confirmacion1.setHeaderText("Esta acción es PERMANENTE e IRREVERSIBLE");
        confirmacion1.setContentText(
            "Si eliminas tu cuenta:\n\n" +
            "❌ Perderás todos tus datos personales\n" +
            "❌ Se eliminarán tus inscripciones a torneos\n" +
            "❌ Perderás tu historial y estadísticas\n" +
            "❌ Se borrarán tus equipos (si eres capitán)\n" +
            "❌ Esta acción NO se puede deshacer\n\n" +
            "¿Estás completamente seguro?"
        );

        ButtonType btnContinuar = new ButtonType("Sí, eliminar mi cuenta", ButtonBar.ButtonData.OK_DONE);
        ...
        confirmacion1.showAndWait().ifPresent(response -> {
            if (response == btnContinuar) {
                ...
                passwordDialog.setTitle("Confirmar con Contraseña");
                passwordDialog.setHeaderText("Ingresa tu contraseña para confirmar la eliminación");
                passwordDialog.setContentText("Contraseña:");

                passwordDialog.showAndWait().ifPresent(password -> {
                    if (password.isEmpty()) {
                        mostrarAlerta("Debes ingresar tu contraseña para confirmar", Alert.AlertType.WARNING);
                        return;
                    }
    ...
    /**
     * Ejecuta la eliminación definitiva de la cuenta
     */
    ...
    private void cerrarSesion() {
        ...
        confirmacion.setContentText("¿Deseas cerrar sesión y volver a la pantalla de inicio?");
